<?php
require_once('header.php');
?>

<div class="pc-container" style="z-index: 0;">
    <div class="pcoded-content" style="z-index: 0;">
        <!-- [ breadcrumb ] start -->

        <div class="page-header-title pb-4" style="z-index: 0;">
            <h4 class="m-b-10" style="z-index: 0;">Active Products</h4>
        </div>
        <!-- [ breadcrumb ] end -->
        <!-- [ Main Content ] start -->
        <div class="row">
            <div class="col-12 card px-3">
                <div class="row px-4 pb-3">
                    <div class="col-12 table-title p-2 px-3" style="background-color: rgba(168, 255, 236, 0.64); color: black;">
                        <div class="row">
                            <?php
                            $active = Database::search("SELECT * FROM `product` WHERE `id`!='0' AND `status_id` = '1' ")->num_rows;
                            $deactive = Database::search("SELECT * FROM `product` WHERE `id`!='0' AND `status_id` = '0' ")->num_rows;
                            ?>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Active Products: <?= $active ?></div>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Rejected Products: <?= $deactive ?></div>
                            <div style="text-align: center;" class="my-auto col-lg-4 col-12"><button class="btn btn-primary" onclick="window.location='newProduct.php';">Add new Product</button></div>
                        </div>
                    </div>
                </div>
                <table class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Image</th>
                            <th>Product</th>
                            <th>Category</th>
                            <th>Discount</th>
                            <th>Price</th>
                            <th>Status</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $resultset = Database::search("SELECT `ref`,`title`,`product`.`status_id`,`image1`,`discountPrice`,`actualPrice`,`product`.`id`, `category`.`name` AS `categoryName`, `sub_category`.`name` AS `sub_categoryName` FROM `product` INNER JOIN `sub_category` ON `sub_category`.`id` = `product`.`sub_category_id` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` WHERE `product`.`id`!='0' AND `product`.`status_id`='1' ORDER BY `add_date` ");
                        $rows = $resultset->num_rows;
                        if ($rows > 0) {

                            for ($x = 1; $x <= $rows; $x++) {
                                $row = $resultset->fetch_assoc();
                                $status = 'checked';
                                if ($row['status_id'] != '1') {
                                    $status = '';
                                }
                        ?>
                                <tr>
                                    <td><?= $x ?></td>
                                    <td><img src="../<?= $row['image1'] ?>" height="60px"></td>
                                    <td> <span class="product-title"><?= $row['title'] ?></span>(<?= $row['sub_categoryName'] ?>)</td>
                                    <td><?= $row['categoryName'] ?></td>
                                    <td>
                                        <?php
                                        if ($row['discountPrice'] == '0') {
                                            echo "N/A";
                                        } else {
                                            echo (100 - number_format(($row['discountPrice'] / $row['actualPrice']) * 100)) . "%";
                                        }
                                        ?>
                                    </td>
                                    <td>
                                        <?php
                                        if ($row['discountPrice'] == '0') {
                                            echo number_format($row['actualPrice'], 2);
                                        } else {
                                            echo number_format($row['discountPrice'], 2);
                                        }
                                        ?>/=
                                    </td>
                                    <td>
                                        <label class="container">
                                            <input id="toggleStatus<?= $row['id'] ?>" type="checkbox" onclick="changeStatusProduct2('<?= $row['id'] ?>');" <?= $status ?>>
                                            <div class="checkmark"></div>
                                        </label>
                                    </td>
                                    <td><img onclick="window.location='updateProduct.php?id=<?= $row['ref'] ?>';" style="cursor: pointer;" height="30px" src="data:image/png;base64,"></td>
                                </tr>
                        <?php
                            }
                        }

                        ?>

                    </tbody>
                </table>
            </div>
        </div>
        <!-- [ Main Content ] end -->

        <!-- -----------------------modal---------------------- -->
        <div id="modal" class="modal1" style="z-index: 0;">
            <!-- <div> -->
            <div class="col-12 col-md-6 offset-md-3 modal--content">
                <div class="row" id="modalContent">

                </div>
            </div>
            <!-- </div> -->
        </div>

    </div>
</div>

<?php require_once('footer.php'); ?>