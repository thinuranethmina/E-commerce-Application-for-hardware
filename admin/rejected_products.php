...
<div class="pc-container">
    <div class="pcoded-content">
        ...
        <div class="page-header-title pb-4">
            <h4 class="m-b-10">Rejected Products</h4>
        </div>
        <!-- [ breadcrumb ] end -->
        <!-- [ Main Content ] start -->
        <div class="row">
            <div class="col-12 card px-3">
                <div class="row px-4 pb-3">
                    <div class="col-12 table-title p-2 px-3" style="background-color: rgba(168, 255, 236, 0.64); color: black;">
                        <div class="row">
                            <?php
                            $active = Database::search("SELECT * FROM `product` WHERE `id`!='0' AND `status_id` = '1' ")->num_rows;
                            $deactive = Database::search("SELECT * FROM `product` WHERE `id`!='0' AND `status_id` = '0' ")->num_rows;
                            ?>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Active Products: <?= $active ?></div>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Rejected Products: <?= $deactive ?></div>
                            <div style="text-align: center;" class="my-auto col-lg-4 col-12"><button class="btn btn-primary" onclick="window.location='newProduct.php';">Add new Product</button></div>
                        </div>
                    </div>
                </div>
                <table class="table table-striped" style="width:100%">
                    ...
                    <tbody>
                        <?php
                        $resultset = Database::search("SELECT `ref`,`title`,`product`.`status_id`,`image1`,`discountPrice`,`actualPrice`,`product`.`id`, `category`.`name` AS `categoryName`, `sub_category`.`name` AS `sub_categoryName` FROM `product` INNER JOIN `sub_category` ON `sub_category`.`id` = `product`.`sub_category_id` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` WHERE `product`.`id`!='0' AND `product`.`status_id`='0' ORDER BY `add_date` ");
                        ...
                        ?>
                                <tr>
                                    <td><?= $x ?></td>
                                    <td><img src="../<?= $row['image1'] ?>" height="60px"></td>
                                    <td> <span class="product-title"><?= $row['title'] ?></span>(<?= $row['sub_categoryName'] ?>)</td>
                                    ...
                                    <td>
                                        ...
                                        if ($row['discountPrice'] == '0') {
                                            ...
                                            echo (100 - number_format(($row['discountPrice'] / $row['actualPrice']) * 100)) . "%";
                                        }
                                        ?>
                                    </td>
                                    <td>
