...
<div class="pc-container" style="z-index: 0;">
    <div class="pcoded-content" style="z-index: 0;">
        ...
        <div class="page-header-title pb-4" style="z-index: 0;">
            <h4 class="m-b-10" style="z-index: 0;">Active Brands</h4>
        </div>
            ...
            <div class="col-12 card px-3">
                <div class="row px-4 pb-3">
                    <div class="col-12 table-title p-2 px-3" style="background-color: rgba(171, 255, 191, 0.8); color: black;">
                        <div class="row">
                            <?php
                            $active = Database::search("SELECT * FROM `brand` WHERE `id`!='0' AND `status_id` = '1' ORDER BY `name` ")->num_rows;
                            $deactive = Database::search("SELECT * FROM `brand` WHERE `id`!='0' AND `status_id` = '0' ORDER BY `name` ")->num_rows;
                            ?>
                            ...
                            <div style="text-align: center;" class="my-auto col-lg-4 col-12"><button class="btn btn-primary" onclick="window.location='add_new_brand.php';">Add new Brand</button></div>
                        </div>
                    </div>
                </div>
                <table class="table table-striped" style="width:100%">
                    ...
                    <tbody>
                        <?php
                        $resultset = Database::search("SELECT * FROM `brand` WHERE `id`!='0' AND `status_id`='1' ORDER BY `name` ");
