...
<div class="pc-container" style="z-index: 0;">
    <div class="pcoded-content" style="z-index: 0;">
        ...
        <div class="page-header-title pb-4" style="z-index: 0;">
            <h4 class="m-b-10" style="z-index: 0;">Active Categories</h4>
        </div>
        ...
        <div class="row px-4">
            <div class="col-12 card px-3">
                <div class="row px-4 pb-3">
                    <div class="col-12 table-title p-2 px-3" style="background-color: rgba(171, 255, 191, 0.8); color: black;">
                        <div class="row">
                            <?php
                            $active = Database::search("SELECT * FROM `sub_category` WHERE `id`!='0' AND `status_id` = '1' ORDER BY `name` ")->num_rows;
                            $deactive = Database::search("SELECT * FROM `sub_category` WHERE `id`!='0' AND `status_id` = '0' ORDER BY `name` ")->num_rows;
                            ?>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Active Sub Categories: <?= $active ?></div>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Rejected Sub Categories: <?= $deactive ?></div>
                            <div style="text-align: center;" class="my-auto col-lg-4 col-12"><button class="btn btn-primary" onclick="window.location='add_new_sub_categories.php';">Add new Sub Category</button></div>
                        </div>
                    </div>
                </div>
                <table class="table table-striped" style="width:100%">
                    ...
                    <tbody>
                        <?php
                        $resultset = Database::search("SELECT `sub_category`.`id`, `sub_category`.`name`, `category`.`name` AS `categoryName`, `sub_category`.`status_id` FROM `sub_category` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` WHERE `sub_category`.`id`!='0' AND `sub_category`.`status_id`='1' ORDER BY `category_id` ");
