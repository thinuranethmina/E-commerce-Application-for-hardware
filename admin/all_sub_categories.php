<?php
require_once('header.php');
?>

<div class="pc-container" style="z-index: 0;">
    <div class="pcoded-content" style="z-index: 0;">
        <!-- [ breadcrumb ] start -->

        <div class="page-header-title pb-4" style="z-index: 0;">
            <h4 class="m-b-10" style="z-index: 0;">All Categories</h4>
        </div>
        <!-- [ breadcrumb ] end -->
        <!-- [ Main Content ] start -->
        <div class="row">
            <div class="col-12 card px-3">
                <div class="row px-4 pb-3">
                    <div class="col-12 table-title p-2 px-3" style="background-color: rgba(168, 255, 236, 0.64); color: black;">
                        <div class="row">
                            <?php
                            $active = Database::search("SELECT * FROM `sub_category` WHERE `id`!='0' AND `status_id` = '1' ORDER BY `name` ")->num_rows;
                            $deactive = Database::search("SELECT * FROM `sub_category` WHERE `id`!='0' AND `status_id` = '0' ORDER BY `name` ")->num_rows;
                            ?>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Active Sub Categories: <?= $active ?></div>
                            <div style="text-align: center;" class="my-auto col-4 d-none d-lg-block">Rejected Sub Categories: <?= $deactive ?></div>
                            <div style="text-align: center;" class="my-auto col-lg-4 col-12"><button class="btn btn-primary" onclick="window.location='add_new_sub_categories.php';">Add new Sub Category</button></div>
                        </div>
                    </div>
                </div>
                <table class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
                            <th>Status</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $resultset = Database::search("SELECT `sub_category`.`id`, `sub_category`.`name`, `category`.`name` AS `categoryName`, `sub_category`.`status_id` FROM `sub_category` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` WHERE `sub_category`.`id`!='0' ORDER BY `category_id` ");
                        $rows = $resultset->num_rows;
                        if ($rows > 0) {

                            for ($x = 1; $x <= $rows; $x++) {
                                $row = $resultset->fetch_assoc();
                                $status = 'checked';
                                if ($row['status_id'] != '1') {
                                    $status = '';
                                }
                        ?>
                                <tr>
                                    <td><?= $x ?></td>
                                    <td><?= $row['name'] ?> (<?= $row['categoryName'] ?>)</td>
                                    <td>
                                        <label class="container">
                                            <input id="toggleStatus<?= $row['id'] ?>" type="checkbox" onclick="changeStatusSubCategory('<?= $row['id'] ?>');" <?= $status ?>>
                                            <div class="checkmark"></div>
                                        </label>
                                    </td>
                                    <td><img onclick="modalOpenSubCategory('<?= $row['id'] ?>');" style="cursor: pointer;" height="30px" src="data:image/png;base64,"></td>
                                </tr>
                        <?php
                            }
                        }

                        ?>

                    </tbody>
                </table>
            </div>
        </div>
        <!-- [ Main Content ] end -->

        <!-- -----------------------modal---------------------- -->
        <div id="modal" class="modal1" style="z-index: 0;">
            <!-- <div> -->
            <div class="col-12 col-md-6 offset-md-3 modal--content">
                <div class="row" id="modalContent">

                </div>
            </div>
            <!-- </div> -->
        </div>

    </div>
</div>

<?php require_once('footer.php'); ?>